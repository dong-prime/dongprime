...
## 11. 결정 필요 사항 (빌드 전에 확정)
1. **인증 방식**: 비밀번호 게이트(MVP) vs Google 로그인(OIDC) vs Supabase admin 역할 — 추천: 비밀번호 시작.
2. **배포처**: Streamlit Community Cloud(추천) vs 기타.
3. **시트 처리**: 대시보드로 일원화 vs 시트 병행(읽기 전용).
4. **이미지 관리**: 상품 이미지를 Storage 업로드로 전환할지(추천) vs 현행 정적 유지.
5. **설정 연동**: 계좌/WhatsApp을 settings 테이블로 옮겨 스토어프론트가 읽게 할지(추천, Phase 2).
6. **범위**: MVP부터 단계적 vs 처음부터 풀세트.
